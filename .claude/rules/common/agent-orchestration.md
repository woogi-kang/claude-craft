...
2. 도메인 에이전트가 필요한 복합 작업인가? → Agent 도구로 위임
3. 스킬 자동 트리거에 해당하는가? → Skill 도구로 실행
4. 단순 코드 작업인가? → 직접 처리 (Glob/Grep/Read/Edit)
5. 탐색이 필요한가? → Explore 서브에이전트
```
...
| 로고, 브랜드 아이덴티티, CIP, 배너, 아이콘 | `design` | 종합 디자인 (55 로고 스타일, CIP 등) |
