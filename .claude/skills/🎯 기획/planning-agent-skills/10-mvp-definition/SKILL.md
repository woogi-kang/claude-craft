...
**Q1. 검증하려는 핵심 가설이 명확한가요?**
- 명확함 → 가설 검증에 필요한 최소 기능 도출
- 모호함 → "이 MVP로 무엇을 알고 싶으세요?"

**Q2. 기능 목록이 있나요?**
- 있음 → MoSCoW 우선순위 분류 진행
- 없음 → "사용자가 목표를 달성하는 데 필요한 단계를 나열해주세요"

**Q3. Must Have가 5개 이하인가요?**
- 예 → 각 기능의 필요성 재검토
- 아니오 → "정말 첫 버전에 모두 필요한가요? 가장 중요한 3개는?"

### 의사결정 포인트

| 시점 | 확인 내용 | 사용자 프롬프트 |
|------|----------|----------------|
| 기능 분류 | Must Have 기준 | "이 기능 없이 가설 검증이 불가능한가요?" |
| 범위 확정 | 최소성 | "더 줄일 수 있는 기능은 없나요?" |
| 성공 기준 | 측정 가능성 | "성공을 어떤 숫자로 판단할까요?" |
| 킬 스위치 | 피벗 기준 | "어떤 결과가 나오면 방향을 바꿀까요?" |
