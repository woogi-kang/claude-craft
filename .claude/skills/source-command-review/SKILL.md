...
description: "멀티-LLM 리뷰 - Codex, Gemini, Codex가 함께 리뷰하고 합의된 피드백 생성"
---
...
여러 LLM(Codex, Gemini, Codex)이 콘텐츠를 병렬로 리뷰하고, 합의 기반의 통합 피드백 리포트를 생성합니다.
...
| Codex만 가용 | 사용자에게 선택지 제공 (설치 or 단독 진행) |
...
| LLM | 역할 | 전문 분야 |
|-----|------|----------|
| **Codex** | 시니어 테크니컬 라이터 | 가독성, 논리 구조, UX |
...
### Codex CLI
```bash
Codex -p "{full_prompt}" --output-format json --max-turns 1
```
...
**참여 LLM**: Codex ✅ | Gemini ✅ | Codex ✅

## 🎯 Executive Summary
| 기준 | Codex | Gemini | Codex | 평균 | 합의도 |
|------|--------|--------|-------|------|--------|

## 🗳️ LLM 합의 매트릭스
| ID | 이슈 | Codex | Gemini | Codex | 합의 | 채택 |
