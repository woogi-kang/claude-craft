...
| AI 엔진 | API | 측정 방식 | 인용 특성 |
|---------|-----|----------|----------|
| **ChatGPT** | OpenAI API (GPT-4o) | 응답 텍스트에서 병원명 매칭 | 간접 인용, 출처 링크 때때로 |
| **Google Gemini** | Gemini API | 응답 + Grounding Sources 분석 | Google Search 기반, AI Overview 대리 지표 |
| **Perplexity** | pplx-api | 응답 + **명시적 출처 링크** 추출 | 가장 추적 용이, 출처 URL 직접 제공 |
| **Google AI Overview** | SerpAPI / Custom Scraper | 검색 결과의 AI Overview 스니펫 분석 | 출처 카드 형태 |

**채점 (6점 만점)**:

| 세부 항목 | 배점 | 판정 |
|-----------|------|------|
| ChatGPT 언급 | 1.5점 | 직접 언급(1.5) / 간접(0.75) / 미언급(0) |
| Gemini 언급 | 1.5점 | 동일 |
| Perplexity 언급 | 1.5점 | 직접 언급 + 출처 링크(1.5) / 언급만(1.0) / 미언급(0) |
| Google AI Overview | 1.5점 | 출처 카드 포함(1.5) / 텍스트만(0.75) / 미노출(0) |

| 등급 | 점수 | 조건 |
|------|------|------|
| Pass | 100% | 4개 엔진 중 3개+에서 직접 언급 |
| Warn | 50% | 1-2개에서 언급 / 또는 간접 언급만 |
| Fail | 0% | 어떤 엔진에서도 미언급 |

**비용**: 진단 1회당 ~$1-2 (API 호출)
**시간**: 2-3분 (병렬 처리)
...
| AI 검색 시뮬레이션 | ~60회 (4엔진 × 5질문 × 3회) | ~$1.50 | 4 |
| **진단 1회 총 비용** | | **~$2-3** | |
| **소요 시간** | | **~60초** (병렬) | |
