...
X(Twitter) 자동 아웃리치 파이프라인 -- **마스터 계정**을 통해 한국 피부과 의료관광 정보를 일본 사용자에게 제공하는 시스템.
...
# 마스터 계정 자격 증명 (답글 및 DM 발송용)
MASTER_X_USERNAME=your_master_username
MASTER_X_PASSWORD=your_master_password
...
| `MASTER_X_USERNAME` | DM 활성화 시 | 마스터 계정 사용자명 |
| `MASTER_X_PASSWORD` | DM 활성화 시 | 마스터 계정 비밀번호 |
...
| 마스터 계정 | 답글 및 DM 발송 | 4-5단계 (답글, DM) |

이 분리는 공격적인 검색 패턴으로 인한 마스터 계정의 레이트 리미팅 또는 정지 위험을 방지합니다.
...
모든 템플릿은 마스터 계정의 목소리에 맞는 자연스러운 일본어입니다:
