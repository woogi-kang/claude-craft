...
X(Twitter) 자동 아웃리치 파이프라인 -- **5개 페르소나 계정**을 통해 한국 피부과 의료관광 정보를 일본 사용자에게 제공하는 시스템.
...
```
                          ...
                          +--------+----------+
                                   ...
                                   v
+--------+  +--------+  +--------+  +--------+  +--------+  +--------+  +--------+
|  검색   |->|  수집   |->|  분석   |->|  양성   |->|  답글   |->|   DM   |->| 포스팅  |
| Search  |  |Collect |  |Analyze |  |Nurture |  | Reply  |  |   DM   |  |Posting |
+--------+  +--------+  +--------+  +--------+  +--------+  +--------+  +--------+
    |            |            |            |            |            |          |
    v            v            v            v            v            v          v
Playwright  PostgreSQL   Codex CLI   Playwright   Playwright   Playwright  Playwright
 (크롤링)     (저장)      (분류)    (팔로우/좋아요) (게시)       (발송)     (트윗 게시)
    |            |            |            |            |            |          |
    +-----+------+-----+------+-----+------+-----+------+-----+-----+----+----+
          |                   |                   |                        |
          v                   v                   v                        v
     outreach_shared     structlog           긴급 정지               페르소나
     (브라우저 스텔스,    (구조화 로깅)        시스템               (5개 고유 목소리)
      인간 시뮬레이션,
      DB, 레이트 리미터)
```

파이프라인은 7단계 순차 프로세스로 실행됩니다. 각 페르소나 계정이 모든 단계를 독립적으로 처리합니다. 각 단계는 공유 PostgreSQL 데이터베이스에 읽기/쓰기하며, 브라우저 단계는 X 웹 인터페이스에 대한 Playwright 브라우저 자동화를 사용합니다.
...
Playwright 브라우저 자동화로 일본어 피부과 키워드에 매칭되는 트윗을 크롤링합니다. **페르소나 계정**이 직접 검색합니다.
...
2. **Codex CLI 분류** -- 트윗 내용, 작성자 메타데이터, 인게이지먼트 지표를 Codex CLI (gpt-5.1-codex-mini)에 JSON 모드로 전송. `intent_type`, `confidence`, `llm_decision`, `rationale` 반환.

신뢰도 임계값(기본 0.7) 미만 분류는 자동 거부됩니다.

### 4단계: 양성 (Nurture)

분석 통과 후 즉시 답글하지 않고, 확률 기반으로 대상 사용자에게 자연스러운 사전 인게이지먼트를 수행합니다.

- **팔로우:** 확률 기반 팔로우 (설정 가능)
- **좋아요:** 대상 트윗에 대한 확률 기반 좋아요 (설정 가능)
- 답글/DM 이전에 관계 기반을 구축
- 자연스러운 계정 활동 패턴 유지
...
- **페르소나 계정** 사용 (각 계정의 고유한 목소리로 생성)
- 2단계 콘텐츠 생성: 전문가 기반 콘텐츠 + 페르소나 어조 적용
- 일일 답글 제한 적용 (기본 50건)
...
페르소나 계정의 타임라인에 독립적인 트윗을 게시합니다.

- **캐주얼 트윗:** 일상적인 화제, 라이프스타일 (전문가 단계 생략)
- **지식 포스트:** 한국 피부과 정보, 시술 팁 (전문가 기반 콘텐츠)
- 계정의 자연스러운 활동 패턴 유지
- 페르소나별 고유한 톤과 주제
...
```
    ...
    src/
        ...
        persona.py            # 5개 페르소나 정의 및 로딩
        ai/
            classifier.py     # 5카테고리 트윗 분류기 (키워드 + Codex CLI)
            content_gen.py    # 2단계 콘텐츠 생성 (Codex CLI 기본)
        ...
        pipeline/
            ...
            halt.py           # 긴급 정지 시스템
            nurture.py        # 팔로우/좋아요 확률 기반 양성
            posting.py        # 캐주얼 트윗 및 지식 포스트 게시
    ...
    launchd/
        com.xoutreach.master.plist      # macOS launchd 데몬 설정
    tests/
        conftest.py           # 공유 픽스처
...
packages/_shared/                # 크로스 플랫폼 공유 라이브러리 (outreach_shared)
    outreach_shared/
        ai/llm_client.py     # LLM Protocol + CodexClient + GeminiCLIClient + FallbackLLMClient
...
- **Codex CLI** (기본 LLM, API 키 불필요)
- **Gemini CLI** (폴백 LLM, API 키 불필요)
...
pip install -e packages/_shared
...
```
# 페르소나 계정 자격 증명 (각 계정이 모든 단계 처리)
MASTER_A_USERNAME=your_master_a_username
MASTER_A_PASSWORD=your_master_a_password
MASTER_B_USERNAME=your_master_b_username
MASTER_B_PASSWORD=your_master_b_password
MASTER_C_USERNAME=your_master_c_username
MASTER_C_PASSWORD=your_master_c_password
MASTER_D_USERNAME=your_master_d_username
MASTER_D_PASSWORD=your_master_d_password
MASTER_E_USERNAME=your_master_e_username
MASTER_E_PASSWORD=your_master_e_password

# GEMINI_API_KEY (선택, Gemini SDK 사용 시에만 필요)
...
posting:
  enabled: true               # 포스팅 기능 활성화
  daily_casual_limit: 3       # 일일 캐주얼 트윗 제한
  daily_knowledge_limit: 2    # 일일 지식 포스트 제한
...
llm:
  provider: codex                  # Codex CLI (gpt-5.1-codex-mini)
  fallback_provider: gemini_cli    # Gemini CLI (gemini-3-flash-preview)
```
...
| 변수 | 필수 | 설명 |
|------|------|------|
| `MASTER_A_USERNAME` | 필수 | master_a (p01_price / みく) 사용자명 |
| `MASTER_A_PASSWORD` | 필수 | master_a 비밀번호 |
| `MASTER_B_USERNAME` | 필수 | master_b (p02_beginner_guide / あや) 사용자명 |
| `MASTER_B_PASSWORD` | 필수 | master_b 비밀번호 |
| `MASTER_C_USERNAME` | 필수 | master_c (p03_procedure_explainer / りこ) 사용자명 |
| `MASTER_C_PASSWORD` | 필수 | master_c 비밀번호 |
| `MASTER_D_USERNAME` | 필수 | master_d (p04_risk_care / なつみ) 사용자명 |
| `MASTER_D_PASSWORD` | 필수 | master_d 비밀번호 |
| `MASTER_E_USERNAME` | 필수 | master_e (p05_lifestyle / ゆい) 사용자명 |
| `MASTER_E_PASSWORD` | 필수 | master_e 비밀번호 |
| `GEMINI_API_KEY` | 선택 | Gemini SDK 사용 시에만 필요 |
...
```bash
# 전체 파이프라인: 검색 -> 수집 -> 분석 -> 양성 -> 답글 -> DM -> 포스팅
...
현재 통계를 표시합니다: 수집된 트윗, 분석 완료, 답글 발송, DM 발송, 포스팅, 오류 수.
...
모든 사전 요구사항을 검증합니다: 환경 변수, 데이터베이스 연결, Playwright 브라우저, Codex CLI 접근.
...
**상태 라이프사이클:** `collected` -> `analyzed` -> `nurtured` -> `replied` -> `dm_sent`
...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    platform        TEXT NOT NULL,              -- 'x'
    account_type    TEXT NOT NULL,              -- 'outreach'
    username        TEXT,
...
파이프라인은 5개의 페르소나 계정을 사용하며, 각 계정이 모든 파이프라인 단계를 독립적으로 처리합니다:

| 계정 | 페르소나 | 전문 영역 | 처리 단계 |
|------|----------|-----------|-----------|
| master_a | p01_price (みく) | 가격 분석 전문 | 전체 7단계 |
| master_b | p02_beginner_guide (あや) | 초보자 가이드 | 전체 7단계 |
| master_c | p03_procedure_explainer (りこ) | 시술 설명 전문 | 전체 7단계 |
| master_d | p04_risk_care (なつみ) | 리스크/사후관리 | 전체 7단계 |
| master_e | p05_lifestyle (ゆい) | 라이프스타일 | 전체 7단계 |

5개 페르소나 계정은 각각 고유한 목소리와 전문 영역을 가지며, 검색부터 포스팅까지 모든 단계를 단일 계정으로 처리합니다. 자세한 내용은 `personas/` 디렉토리를 참조하세요.
...
**2단계: Codex CLI 분류** (`src/ai/classifier.py`)

키워드 필터링을 통과한 트윗은 Codex CLI (gpt-5.1-codex-mini)로 심층 분류됩니다:
...
모든 답글과 DM 콘텐츠는 2단계 프로세스로 생성됩니다:

**Phase 1: 전문가 콘텐츠 (Expert)**

한국 피부과 전문가가 사실에 기반한 정확한 베이스 콘텐츠를 생성합니다:
- 시술 정보, 가격 범위, 클리닉 비교 등 팩트 기반
- 의학적으로 정확한 정보만 포함
- 페르소나와 무관한 중립적 톤

**Phase 2: 페르소나 적용 (Persona)**

LLM이 베이스 콘텐츠를 페르소나의 고유한 목소리로 변환합니다:
- 각 페르소나의 말투 (어미, 어휘, 이모지 사용 패턴)
- 캐릭터에 맞는 톤 조정
- 전문 영역에 따른 강조점 변경

**캐주얼 트윗 예외:** 7단계 포스팅의 캐주얼 일상 트윗은 전문가 단계를 생략하고 페르소나 단계만 실행합니다.

### LLM 구성

LLM은 CLI 기반으로 동작하며 API 키가 필요하지 않습니다:

- **기본:** Codex CLI (`gpt-5.1-codex-mini`) -- API 키 불필요
- **폴백:** Gemini CLI (`gemini-3-flash-preview`) -- API 키 불필요
- 기본 LLM 실패 시 자동으로 폴백 전환
- `FallbackLLMClient`가 자동 전환 관리
...
모든 템플릿은 페르소나별 목소리에 맞는 자연스러운 일본어입니다:
...
macOS 백그라운드 서비스로 데몬을 실행하기 위한 `launchd` plist가 `launchd/com.xoutreach.master.plist`에 포함되어 있습니다.
...
테스트 스위트는 20개 테스트 파일에 걸쳐 **367개 테스트**를 포함합니다.
...
| `test_dm_passcode.py` | 단위 | DM 암호화 패스코드 |
| `test_halt.py` | 단위 | 긴급 정지 시스템 |
| `test_nurture.py` | 단위 | 팔로우/좋아요 양성 |
| `test_posting.py` | 단위 | 캐주얼 트윗 포스팅 |
| `test_reply.py` | 단위 | 답글 생성 및 발송 |
| `test_dm.py` | 단위 | DM 생성 및 발송 |
| `test_persona.py` | 단위 | 페르소나 로딩 및 검증 |
...
Codex CLI가 설치되어 있지 않거나 PATH에 없습니다. Codex CLI를 설치하고 `codex` 명령이 실행 가능한지 확인하세요. 폴백으로 Gemini CLI가 사용됩니다.

### "Gemini CLI not found" (폴백 실패 시)

기본 LLM(Codex CLI)과 폴백(Gemini CLI) 모두 사용할 수 없습니다. 최소 하나의 CLI를 설치하세요:
...
1. 브라우저를 수동으로 열고 해당 계정에 로그인
2. 인증 챌린지를 완료
3. 세션 캐시 삭제: `rm -rf data/sessions/{account_name}/`
4. 파이프라인 재시도
...
Codex CLI 모델이 많은 트윗에 대해 불확실합니다. 가능한 원인:
...
| 도구 | 용도 |
|------|------|
| `codex` (Codex CLI) | 기본 LLM -- gpt-5.1-codex-mini, API 키 불필요 |
| `gemini` (Gemini CLI) | 폴백 LLM -- gemini-3-flash-preview, API 키 불필요 |
