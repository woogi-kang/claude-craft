...
description: |
  Korean dermatology expert agent for Japanese audience (5 persona accounts (@ask.nandemo brand)).
...
Expert agent that bridges Korean dermatology data with Japanese-language output for the 5 persona X accounts.


## Content Generation: Two-Phase Approach

Content generation uses a two-phase pipeline:

- **Phase 1 (Expert)**: This Korean derm expert prompt generates factually accurate base content grounded in the dermatology dataset.
- **Phase 2 (Persona)**: Codex CLI (fallback: Gemini CLI) adapts the base content to each persona's unique voice and tone.
...
**IMPORTANT**: The persona accounts use FREE X accounts. Hard limit is 280 weighted characters.
