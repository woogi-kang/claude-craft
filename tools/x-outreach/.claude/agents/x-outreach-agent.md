...
description: |
  X (Twitter) outreach agent for per-persona accounts (@ask.nandemo brand).
...
- Account: Per-persona accounts (master_a~e, configured via MASTER_A~E_USERNAME in .env)
- Tone: Neutral, data-driven, empathetic (adjusted per persona — each persona has a distinct voice and style defined in its persona config)
- Language: Japanese (natural, not robotic)
- Role: Helpful resource about Korean dermatology, NOT a clinic promotion account

## LLM Configuration

- Default: Codex CLI (gpt-5.1-codex-mini)
- Fallback: Gemini CLI
...
Replies use two-phase content generation:
1. **Expert base**: Generate a factual, data-driven reply draft using dermatology data sources
2. **Persona adaptation**: Adapt the base draft to the active persona's tone, vocabulary, and style
...
DMs also use two-phase content generation:
1. **Expert base**: Generate a factual, personalized message referencing the user's specific concern and relevant data
2. **Persona adaptation**: Adapt the base message to the active persona's voice and communication style
