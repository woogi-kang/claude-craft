"""Read message content from within a KakaoTalk chatroom.
...
class MessageReader:
    """Extract messages from the currently open KakaoTalk chatroom.
    ...
    def read_messages(self, count: int = 5) -> list[dict[str, str | bool | None]]:
        ...
        try:
            text_elements = self._device.resourceId(selectors.MESSAGE_TEXT)  # type: ignore[attr-defined]
            ...
            for i in range(start_index, total):
                try:
                    ...
                    # In KakaoTalk, incoming messages have a profile_name sibling;
                    ...
                    try:
                        ...
                        sender_el = parent.child(resourceId=selectors.MESSAGE_SENDER)
                        if sender_el.exists:
    ...
    def get_chatroom_title(self) -> str:
        ...
        """
        try:
            title_el = self._device.resourceId(selectors.CHATROOM_TITLE)  # type: ignore[attr-defined]
            if title_el.exists:
