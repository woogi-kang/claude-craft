...
class ReservationWorker:
    ...
    contactor:
        KakaoTalk deep-link opener (None for dry-run mode).
    clinic_lookup:
        Clinic database for resolving KakaoTalk URLs.
    poll_interval:
    ...
    async def _handle_created(self, reservation: dict[str, Any]) -> None:
        ...
        rid = reservation["id"]
        kakao_url = reservation.get("clinic_kakao_url")
        ...
        if not kakao_url:
            ...
            if clinic_id:
                ...
                if clinic and clinic.kakao_url:
                    kakao_url = clinic.kakao_url
                    self._repo.update_reservation(rid, clinic_kakao_url=kakao_url)

        if not kakao_url:
            logger.warning("no_kakao_url", reservation_id=rid)
            self._repo.update_reservation(
                rid,
                status="failed",
                error_message="No KakaoTalk channel URL available",
            )
            return

        self._repo.update_reservation(rid, status="contacting")
        logger.info("reservation_contacting", reservation_id=rid)
    ...
    async def _handle_contacting(self, reservation: dict[str, Any]) -> None:
        ...
        # Open KakaoTalk chat (skip in dry-run mode)
        if self._contactor:
            ...
            if not opened:
                self._repo.update_reservation(
                    rid,
                    status="failed",
                    error_message="Failed to open KakaoTalk chat",
                )
                return
