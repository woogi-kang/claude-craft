...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reservation Bot")
    parser.add_argument(
    ...
    parser.add_argument(
        "--platform", default=None, help="Messenger platform: kakao or line (default: from config)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Skip actual messenger interaction"
    )
    args = parser.parse_args()

    if args.process:
        asyncio.run(_process_single(args.process, dry_run=args.dry_run, platform=args.platform))
        return

    if args.worker:
        asyncio.run(_run_worker_standalone(dry_run=args.dry_run, platform=args.platform))
        return
...
def _build_worker(*, dry_run: bool = False, platform: str | None = None):
    ...
    messenger_platform = platform or settings.reservation.messenger_platform
    ...
    # In dry-run mode, contactor is None (no messenger interaction)
    contactor = None
    if not dry_run:
        try:
            ...
            from src.messenger.selectors import get_selectors
            from src.outbound.contact import ClinicContactor

            platform_selectors = get_selectors(messenger_platform)
            device = u2.connect(settings.emulator.serial)
            navigator = Navigator(device, selectors=platform_selectors)
            ...
            print("Running in dry-run mode (no messenger interaction)")
...
async def _run_worker_standalone(*, dry_run: bool = False, platform: str | None = None) -> None:
    """Run the background worker as a standalone process."""
    worker = _build_worker(dry_run=dry_run, platform=platform)
...
async def _process_single(request_id: str, *, dry_run: bool = False, platform: str | None = None) -> None:
    ...
    plat = reservation.get("contact_platform", platform or "kakao")
    print(f"Reservation: {reservation['clinic_name']} / {reservation['procedure_name']}")
    print(f"Status: {reservation['status']} | Platform: {plat}")
    print(f"Patient: {reservation['patient_name']} ({reservation['patient_nationality']})")

    if dry_run:
        print(f"[DRY RUN] Would contact clinic via {plat}")
        print(f"  Contact URL: {reservation.get('clinic_contact_url', 'N/A')}")
        return

    worker = _build_worker(dry_run=False, platform=platform)
