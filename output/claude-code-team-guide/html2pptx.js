...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      // Detect vertical centering: when line-height matches element height
      const lineHeightPx = parseFloat(computed.lineHeight);
      const elementHeightPx = h;
      const isSingleLineVerticalCenter =
        !isNaN(lineHeightPx) &&
        Math.abs(lineHeightPx - elementHeightPx) < 5 &&
        elementHeightPx < lineHeightPx * 2;
      ...
      const baseStyle = {
        ...
        color: rgbToHex(computed.color),
        align: computed.textAlign === 'start' ? 'left' : computed.textAlign,
        valign: isSingleLineVerticalCenter ? 'middle' : 'top',
